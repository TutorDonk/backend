...
export class BookingService {
  ...
  async getCourses(educationLevel: number) {
    ...
    const courses = await this.firestoreService.getDocument(
      'courses',
  ...
  async createBooking(data: any) {
    ...
    this.firestoreService.updateDocument('booking', id, {
      ...data,
      id: id,
      status: 0,
    });
  ...
  async updateBookingStatus(bookingId: string, status: number) {
    await this.firestoreService.updateDocument('booking', bookingId, {
      status,
    });
  }

  async getBooking(idUser: string, role: string) {
    return await this.firestoreService.findDocumentByIdTutorOrIdUser(
      'booking',
      idUser,
      role,
    );
  ...
  async getBookingHistory(idUser: string, status: number) {
    const bookings = await this.firestoreService.findBookingsByStatus('booking', idUser, status);
